...
//     Cookies.set("auth_token", data.access_token, { expires: 7, secure: true });

//     return data;
//   },

//   async signup(data: SignupData): Promise<AuthResponse> {
//     const payload = { email: data.email, full_name: data.Full_Name, password: data.password };
//     const response = await fetch(`${API_BASE_URL}/auth/register`, {
//       method: "POST",
//       headers: { "Content-Type": "application/json" },
//       body: JSON.stringify(payload),
...
//     // Save token in cookie
//     Cookies.set("auth_token", dataResp.access_token, { expires: 7, secure: true });
