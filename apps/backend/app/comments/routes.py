...
def create_comment(
    ...
    if comment_in.task_id != task_id:
        raise HTTPException(
            status_code=400,
            detail="Task ID in body must match the one in URL"
        )
...
def read_comments(
    ...
    limit: int = 100,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),  # optional, depending on access rules
):
