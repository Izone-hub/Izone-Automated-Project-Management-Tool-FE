...
@router.get("/task/{card_id}", response_model=list[AttachmentOut])
def get_task_attachments(card_id: UUID, db: Session = Depends(get_db)):
    return get_attachments_by_task(db, card_id)
